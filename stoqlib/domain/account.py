...
from stoqlib.database.properties import DateTimeCol, UUIDCol
...
class Account(Domain):
    ...
    parent_id = UUIDCol(default=None)

    #: parent account
    parent = Reference(parent_id, 'Account.id')

    station_id = UUIDCol(default=None)
...
class AccountTransaction(Domain):
    ...
    payment_id = UUIDCol(default=None)
